...
The LSH schemes considered in this paper both operate in \textit{Hamming space}. A Hamming space $H(n, q)$ is defined as the set of all words of length $n$ over an alphabet of size $q$. A distance measure over a Hamming space is the number of letters in which two words differ. The alphabet that we consider for LSH is $\{0, 1\}$ and words are therefore bit vectors of length $n$. As such, the distance measure becomes the number of bits that differ between two vectors.
